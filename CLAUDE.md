...
| `components/ui/ChartLoader.vue` | Widget-loader candidates that build a small chart instead of spinning: `morph · trend · orbit · equalizer · slices · scatter · blocks · gauge`, one 160×100 artboard, `--chart-*` palette, `speed` + rotating `caption`, reduced-motion → a gentle breath. Shown at `/loaders` (`views/LoaderGallery.vue`, rail icon under Empty States). Not wired into WidgetCard yet — pick one first. |
