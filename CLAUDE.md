...
| `components/ui/ChartLoader.vue` | Widget-loader candidates that build a small chart instead of spinning: `morph · trend · orbit · equalizer · slices · scatter · blocks · gauge` (morph cycles five charts: columns → trend → donut → pattern → blocks), one 160×100 artboard drawn ≤100px wide, an x-axis line on axis charts only (none under pie/donut), `--chart-*` palette, `speed` + rotating `caption`, reduced-motion → a gentle breath. Shown at `/loaders` (`views/LoaderGallery.vue`, rail icon under Empty States). WidgetCard plays the `morph` loader for a tile with `loaderDemo: true` (6s on mount and on every Refresh) — seeded on "Open Requests By Status" in Helpdesk Overview (Grouped) only. |
